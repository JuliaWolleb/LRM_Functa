...
def trainer(args, train_function, test_function, model_wrapper, meta_optimizer, train_loader, test_loader, logger, scheduler):
    """
    ...
    for it, train_batch in enumerate(train_loader):
        ...
        if args.dimension == '2d':
                if torch.isnan(train_batch).any():
                    continue
                train_batch = train_batch.float().to(device, non_blocking=True) 
        ...
        train_function(args, step, model_wrapper, meta_optimizer, train_batch, metric_logger, logger, scheduler)
